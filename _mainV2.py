...
# creates while true loop to have a continuing loop for the game
    while True:
        #updates all the sprites and add the variables into the group
        all_sprites.update()
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                sys.exit()
            #Ben McCardy wrote this code (line 122-130)
...
# gets a new cannon and add it to the group
def newbomb():
    b = Cannon_ball(screen, game_settings)
    all_sprites.add(b) 
    bombs.add(b)
